    ...
    public boolean run (Player player, String[] args)
      throws CommandException {
        ...
        try {
            ...
            player.sendMessage(
                "Usage: /warp <add|del|set|list|<warp name>> [key] [value]");
            return true;
        };
        String origc = subc;
        ...
        if (subc.equalsIgnoreCase("add")) {
            ...
            if (warp.getProperty(plugin.warpManager.WarpBase(arg)) == null) {
                player.sendMessage(ChatColor.RED +
                    "[Waypoint] There is no warp by that name.");
                return true;
            };
            ...
            player.sendMessage(ChatColor.AQUA +
                "[Waypoint] Warp " + arg + " has been deleted.");
            return true;
        }
        else if (subc.equalsIgnoreCase("remote")) {
            if (arg == null || k == null) {
                player.sendMessage(ChatColor.RED +
                    "/warp remote <player> <warpname>");
                return true;
            };
            ...
            if (!owner.equals(player.getName().toString())) {
                player.sendMessage(ChatColor.RED +
                    "[Waypoint] You do not have permission modify this warp.");
                return true;
            };
            ...
            if (warp.getProperty(plugin.warpManager.WarpNode(arg, "permission")) == null) {
                player.sendMessage(ChatColor.RED +
                    "[Waypoint] This warp does not exist.");
                return true;
            };
            boolean f = plugin.warpManager.SetWarpProp(arg, k, v);
            if (!f) {
                player.sendMessage(ChatColor.RED +
                    "[Waypoint] An error occurred while trying to set the properties of warp: " + arg);
                return true;
            }
            else if (f) {
                player.sendMessage(ChatColor.GREEN +
                    "[Waypoint] Successfully set the property '" + k + "' to '" + v + "' for warp " + arg);
                return true;
            };
            return true;
        }
            ...
            if (!permissions.has(player, "waypoint.warp.list")) {
                player.sendMessage(ChatColor.RED +
                    "[Waypoint] You do not have the permission to access this command.");
                return true;
            };
            int x = 0;
        ...
        // TODO: case sensitivity check
            ...
            player.sendMessage(ChatColor.LIGHT_PURPLE +
                "[Waypoint] Warp " + origc + " does not exist.");
            return true;
        };
    return false;
    };
    ...
    public boolean run (Player player, String[] args)
      throws CommandException {
        ...
        if (subc.equalsIgnoreCase("del")) {
            ...
            player.sendMessage(ChatColor.BLUE +
                "[Waypoint] Metadata for warp '" + arg + "':");
